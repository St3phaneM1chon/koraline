...
export default function PlanComptablePage() {
  ...
  const summaryCardColors: Record<string, { bg: string; border: string; text: string; value: string }> = {
    blue: { bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', text: 'text-indigo-400', value: 'text-[var(--k-text-primary)]' },
    red: { bg: 'bg-red-500/10', border: 'border-red-500/20', text: 'text-red-400', value: 'text-[var(--k-text-primary)]' },
    purple: { bg: 'bg-purple-500/10', border: 'border-purple-500/20', text: 'text-purple-400', value: 'text-[var(--k-text-primary)]' },
    green: { bg: 'bg-green-500/10', border: 'border-green-500/20', text: 'text-green-400', value: 'text-[var(--k-text-primary)]' },
    sky: { bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', text: 'text-indigo-400', value: 'text-[var(--k-text-primary)]' },
  };
  ...
  const renderGifiCell = (account: Account) => {
    if (account.gifiCode) {
      return (
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono text-xs text-[var(--k-text-secondary)]">{account.gifiCode}</span>
          {account.gifiName && (
            <span className="text-xs text-[var(--k-text-tertiary)] truncate max-w-[120px]" title={account.gifiName}>
              {account.gifiName}
            </span>
          )}
        </div>
    ...
    return (
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs text-[var(--k-text-muted)]">&mdash;</span>
      </div>
    );
  };
  ...
  if (loading) return (
    <div aria-live="polite" aria-busy="true" className="p-8 space-y-4 animate-pulse">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {[1,2,3,4,5].map(i => <div key={i} className="h-24 bg-[var(--k-glass-thin)] rounded-xl"></div>)}
      </div>
  ...
  return (
    <div className="space-y-6">
        ...
        {Object.entries(accountTypes).map(([key, value]) => {
          ...
          return (
            <div key={key} className={`${colors.bg} rounded-xl p-4 border ${colors.border}`}>
              ...
              <p className="text-xs text-[var(--k-text-tertiary)] mt-1">
      ...
      <div className="flex items-center gap-3">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#6366f1]/10 border border-indigo-200 rounded-lg text-sm">
          <span className="w-2 h-2 rounded-full bg-[#6366f1]/100" />
          ...
          {totalAccountCount > 0 && (
            <span className="text-[#818cf8] text-xs">
      ...
      <SectionCard title={t('admin.chartOfAccounts.title')} theme={theme} noPadding>
        ...
        <table className="w-full">
          <thead className="bg-[var(--k-bg-surface)]">
            <tr>
              <th scope="col" className="px-4 py-3 text-start text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.code')}</th>
              <th scope="col" className="px-4 py-3 text-start text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.accountName')}</th>
              <th scope="col" className="px-4 py-3 text-start text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.type')}</th>
              <th scope="col" className="px-4 py-3 text-start text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.gifi')}</th>
              <th scope="col" className="px-4 py-3 text-start text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.category')}</th>
              <th scope="col" className="px-4 py-3 text-end text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.balance')}</th>
              <th scope="col" className="px-4 py-3 text-center text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.statusCol')}</th>
              <th scope="col" className="px-4 py-3 text-center text-xs font-semibold text-[var(--k-text-tertiary)] uppercase">{t('admin.chartOfAccounts.actionsCol')}</th>
            </tr>
          </thead>
              ...
              return (
                <>{/* Fragment key on first tr */}
                  <tr key={account.id} className="hover:bg-[var(--k-bg-surface)]">
                      ...
                      <div className="flex items-center gap-2">
                        {hasChildren && (
                          <button
                            ...
                            className="p-0.5 hover:bg-[var(--k-glass-thin)] rounded"
                            aria-label={isExpanded ? 'Reduire' : 'Developper'}
                          >
                            <ChevronRight
                              className={`w-4 h-4 text-[var(--k-text-tertiary)] transition-transform ${isExpanded ? 'rotate-90' : ''}`}
                            />
                          </button>
                        )}
                        <span className="font-mono font-medium text-[var(--k-text-primary)]">{account.code}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="font-medium text-[var(--k-text-primary)]">{account.name}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-[var(--k-text-secondary)] text-sm">{account.category}</td>
                    <td className="px-4 py-3 text-end">
                      <span className={`font-medium ${account.balance >= 0 ? 'text-[var(--k-text-primary)]' : 'text-red-600'}`}>
                  ...
                  {isExpanded && children.map((child) => (
                    <tr key={child.id} className="bg-[var(--k-bg-surface)]/50 hover:bg-[var(--k-glass-thin)]">
                      <td className="px-4 py-2 ps-12">
                        <span className="font-mono text-sm text-[var(--k-text-secondary)]">{child.code}</span>
                      </td>
                      <td className="px-4 py-2">
                        <span className="text-sm text-[var(--k-text-secondary)]">{child.name}</span>
                      </td>
                      <td className="px-4 py-2">
                      ...
                      <td className="px-4 py-2 text-[var(--k-text-tertiary)] text-sm">{child.category}</td>
                      <td className="px-4 py-2 text-end">
                        <span className={`text-sm ${child.balance >= 0 ? 'text-[var(--k-text-secondary)]' : 'text-red-600'}`}>
                      ...
                      <td className="px-4 py-2 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button
                            ...
                            className="p-1 text-[var(--k-text-muted)] hover:text-[#818cf8] hover:bg-[#6366f1]/10 rounded"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField label={t('admin.chartOfAccounts.type')}>
              <select
                ...
                className="w-full h-9 px-3 rounded-lg border border-[var(--k-border-default)] text-sm text-[var(--k-text-primary)] bg-[var(--k-glass-thin)] backdrop-blur-sm
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4 rounded border-[var(--k-border-default)] text-emerald-600"
            />
            <label htmlFor="isActive" className="text-sm text-[var(--k-text-secondary)]">{t('admin.chartOfAccounts.activeAccount')}</label>
          </div>

          {/* --- GIFI / Fiscal Section (only in edit mode) --- */}
          {editingAccount && (
            <>
              <div className="border-t border-[var(--k-border-subtle)] pt-4 mt-4">
                <h3 className="text-sm font-semibold text-[#818cf8] mb-3">
                ...
                <div className="space-y-3">
                  ...
                  {gifiSuggestions.length > 0 && (
                    <div className="border border-indigo-200 rounded-lg overflow-hidden bg-[var(--k-glass-thin)] backdrop-blur-sm shadow-sm">
                      {gifiSuggestions.map((s) => (
                        <button
                          key={s.code}
                          onClick={() => selectGifiSuggestion(s)}
                          className="w-full px-3 py-2 text-start hover:bg-[#6366f1]/10 flex items-center justify-between border-b border-[var(--k-border-subtle)] last:border-b-0"
                        >
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-sm font-medium text-[#818cf8]">{s.code}</span>
                            <span className="text-sm text-[var(--k-text-secondary)]">
                              {locale === 'fr' ? s.nameFr : s.nameEn}
                            </span>
                          </div>
                          <span className={`text-xs px-1.5 py-0.5 rounded ${
                            s.score >= 0.8 ? 'bg-emerald-500/15 text-emerald-400' :
                            s.score >= 0.5 ? 'bg-amber-500/15 text-amber-400' :
                    ...
                    <div className="flex items-end pb-1">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-4 h-4 rounded border-[var(--k-border-default)] text-[#818cf8]"
                        />
                        <label htmlFor="isContra" className="text-sm text-[var(--k-text-secondary)]">
